...
OpenWorld is a Postgres-resident agent harness. Agent state, turns, tool calls,
memory, and outbound delivery all live in PostgreSQL tables under the
`OpenWorld` schema. Agents point
at shared rows in `OpenWorld.models`, so multiple agents can reuse the same model
...
`"own agent" AND chat_id = current_setting('OpenWorld.current_chat_id')`.
...
for primary, and `OpenWorld_service` for the sidecar. The intent is that no
LLM-authored SQL runs with secret access, and that is now enforced for both tool
scopes (the user tiers and `OpenWorld_service`, the latter via the non-secret
...
`OpenWorld_service`, a broad `BYPASSRLS` scope that reads non-secret `config` only
...
The `OpenWorld_dashboard` role (the admin dashboard's DB principal) is `BYPASSRLS`
...
- **Secrets are kept out of every LLM-tool scope.** Agent roles read their own
  ...
  SELECT on non-secret own rows only, and `OpenWorld_service` (the sidecar's
  ...
  `OpenWorld_dashboard` see secrets directly (the dashboard API redacts them).
...
healthy, and runs `psql --no-psqlrc --single-transaction --set=ON_ERROR_STOP=1 --set=... --file=/OpenWorld/agents.sql`.
