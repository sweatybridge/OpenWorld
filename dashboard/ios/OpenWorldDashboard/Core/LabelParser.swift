...
enum LabelType: String {
    case loop, inbox, cron, send, tool, typing, OpenWorld, other
}
...
enum LabelParser {
    ...
    static func parse(_ raw: String) -> ParsedLabel {
        ...
        guard parts.first == "OpenWorld", parts.count >= 2 else {
        ...
        return ParsedLabel(type: .OpenWorld, agent: nil, ref: nil, friendly: raw)
    }

    /// Numeric message id embedded in a traceable label (loop/send/typing/tool),
    /// or nil. Mirrors OpenWorld.parse_instance_label on the server; used to open
    /// the turn-trace view from any of these instances.
    static func messageId(from raw: String) -> Int? {
        guard let regex = try? NSRegularExpression(pattern: "OpenWorld:(?:[^:]+:loop|send|tool|typing):(\\d+)") else { return nil }
