...
export async function getOverviewAgents(): Promise<Array<{ id: number; slug: string; enabled: boolean }>> {
  const { rows } = await query<{ id: number; slug: string; enabled: boolean }>(
    "SELECT id, slug, enabled FROM OpenWorld.agents ORDER BY id"
  );
  return rows;
}
...
export async function listAgents(): Promise<AgentRow[]> {
  const { rows } = await query<AgentRow>(`
    SELECT a.id, a.slug, a.soul, a.enabled, a.max_turn, a.model_id,
           ...
           (SELECT count(*) FROM OpenWorld.messages WHERE agent_id = a.id)::text AS msg_count,
           (SELECT count(*) FROM OpenWorld.memory   WHERE agent_id = a.id)::text AS mem_count,
           (SELECT count(*) FROM df.instances     WHERE label LIKE 'OpenWorld:' || a.slug || ':%')::text AS wf_count
    FROM OpenWorld.agents a
    LEFT JOIN OpenWorld.models m ON m.id = a.model_id
    ORDER BY a.id`);
  return rows;
}
...
export async function listMemory(agentId: number | null) {
  const { rows } = await query(
    `SELECT m.id, m.agent_id, m.content, m.payload, m.enabled, m.created_at, m.updated_at,
     ...
     LEFT JOIN OpenWorld.memory_sources ms ON ms.memory_id = m.id
...
export async function listUsers() {
  const { rows } = await query(
    `SELECT id, channel, external_id, username, display_name, tier, payload, created_at, updated_at
     FROM OpenWorld.users ORDER BY updated_at DESC LIMIT 500`
  );
  return rows;
}
...
// All indexes across the non-system schemas (OpenWorld, OpenWorld_tools, df, ffmpeg,
