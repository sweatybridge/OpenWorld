...
import com.OpenWorld.dashboard.BuildConfig
...
object ApiProvider {
    ...
    private fun api(): OpenWorldApi = retrofit().create(OpenWorldApi::class.java)

    /** Wraps a Retrofit call: maps 401 → [AuthException], everything else → [ApiException]. */
    private suspend fun <T> req(block: suspend OpenWorldApi.() -> T): T = try {
        api().block()
