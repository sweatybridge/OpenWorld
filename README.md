...
- `GENERATE_VIDEO`: generate a short video clip via a configured
  ...
  polling) and send it as a Telegram video. The model passes a prompt (plus
  ...
  returns. Requires `OPENWORLD_SDCPP_API_BASE` and the sdcpp host to be in
  pg_durable's HTTP egress allowlist.
