...
| `61_generate_video.sql` | `GENERATE_VIDEO` sdcpp tool: pure helpers (`_sdcpp_job_terminal`, `_sdcpp_job_url`, `_sdcpp_finalize_video`, `_sdcpp_error_result`) + the graph BUILD (config-gated, endpoint/chat_id baked as literals); no `df.start` needed |
