...
--   OpenWorld.models.id = 1
--   OpenWorld.agents.id = 1 (primary), 2 (sidecar)
...
TRUNCATE OpenWorld.memory_sources, OpenWorld.memory, OpenWorld.messages,
         OpenWorld.config, OpenWorld.users,
         OpenWorld.agents, OpenWorld.models
...
SELECT 'primary', 'primary soul', m.id FROM OpenWorld.models m
WHERE m.name = 'pgtap-model'
ON CONFLICT (slug) DO NOTHING;
INSERT INTO OpenWorld.agents(slug, soul, model_id)
SELECT 'sidecar', 'sidecar soul', m.id FROM OpenWorld.models m
WHERE m.name = 'pgtap-model'
ON CONFLICT (slug) DO NOTHING;
...
FROM OpenWorld.agents a WHERE a.slug IN ('primary', 'sidecar')
...
INSERT INTO OpenWorld.messages(agent_id, role, content, chat_id, channel)
SELECT a.id, 'user', 'user-msg-' || a.slug, 'CZ1', 'telegram'
FROM OpenWorld.agents a WHERE a.slug = 'primary';
INSERT INTO OpenWorld.messages(agent_id, role, content, chat_id, channel)
SELECT a.id, 'assistant', 'asst-msg-' || a.slug, 'CZ1', 'telegram'
FROM OpenWorld.agents a WHERE a.slug = 'primary';
INSERT INTO OpenWorld.messages(agent_id, role, content, chat_id, channel)
SELECT a.id, 'user', 'user-msg-' || a.slug, 'CZ2', 'telegram'
FROM OpenWorld.agents a WHERE a.slug = 'sidecar';

-- memory: one row per agent
INSERT INTO OpenWorld.memory(agent_id, content)
SELECT a.id, 'mem-' || a.slug FROM OpenWorld.agents a
WHERE a.slug IN ('primary', 'sidecar')
ON CONFLICT DO NOTHING;

-- channel identity ledger: one anonymous + one authenticated telegram user
INSERT INTO OpenWorld.users(channel, external_id, username, tier)
VALUES
