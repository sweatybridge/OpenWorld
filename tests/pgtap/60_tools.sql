...
SELECT is(attotools._opt_int(NULL, 'seconds', 7), 7,
  '_opt_int(NULL options) -> default');
SELECT is(attotools._opt_int('{"seconds":"3"}'::jsonb, 'seconds', 7), 3,
  '_opt_int reads a numeric-as-string value');
SELECT is(attotools._opt_float('{"t":"2.5"}'::jsonb, 't', 0.0), 2.5::float8,
  '_opt_float reads a float');
SELECT is(attotools._opt_bool('{"hwaccel":"true"}'::jsonb, 'hwaccel', false), true,
  '_opt_bool reads true');
SELECT is(attotools._opt_text('{"fmt":"jpeg"}'::jsonb, 'fmt', 'png'), 'jpeg',
...
INSERT INTO _qr
SELECT attotools._queue_send('video', decode(:'png','hex'), 'raw', '', '', '')::jsonb;
