...
SELECT is(
  OpenWorld_tools._unwrap_tool_result((SELECT v FROM env)),
  jsonb_build_object('rows', jsonb_build_array(jsonb_build_object('tablename','agents')),
                     'row_count', 1)::text,
...
SELECT is(
  OpenWorld_tools._unwrap_tool_result('{"rows":[{"result":"Linux 6.8.0"}],"row_count":1}'),
  'Linux 6.8.0',
...
SELECT is(
  OpenWorld_tools._unwrap_tool_result('error: tool cancelled'),
...
SELECT throws_ok(
  $$SELECT OpenWorld_tools._hls_media(NULL)$$,
  'playlist_id is required',
  '_hls_media(NULL) raises before any ffmpeg call'
);
SELECT throws_ok(
  $$SELECT OpenWorld_tools._hls_media(999999)$$,
...
SELECT is(OpenWorld_tools._opt_int(NULL, 'seconds', 7), 7,
  '_opt_int(NULL options) -> default');
SELECT is(OpenWorld_tools._opt_int('{"seconds":"3"}'::jsonb, 'seconds', 7), 3,
  '_opt_int reads a numeric-as-string value');
SELECT is(OpenWorld_tools._opt_float('{"t":"2.5"}'::jsonb, 't', 0.0), 2.5::float8,
  '_opt_float reads a float');
SELECT is(OpenWorld_tools._opt_bool('{"hwaccel":"true"}'::jsonb, 'hwaccel', false), true,
  '_opt_bool reads true');
SELECT is(OpenWorld_tools._opt_text('{"fmt":"jpeg"}'::jsonb, 'fmt', 'png'), 'jpeg',
...
INSERT INTO _qr
SELECT OpenWorld_tools._queue_send('video', decode(:'png','hex'), 'raw', '', '', '')::jsonb;
