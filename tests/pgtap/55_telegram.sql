...
-- (df.start-ed as attobot:send:<msg_id>), so when Telegram rejects the send it
...
SELECT throws_ok(
  $$SELECT attobot.send_message('primary', 0, '{"status":400,"body":"{\"ok\":false,\"error_code\":400,\"description\":\"Bad Request: chat not found\"}"}'::jsonb)$$,
  'P0001'::char(5),
  NULL,
...
SELECT is(
  (SELECT (attobot.send_message('primary', 0, '{"status":200,"body":"{\"ok\":true,\"result\":{\"message_id\":42}}"}'::jsonb))->>'sent'),
  'true',
