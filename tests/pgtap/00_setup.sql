...
-- docker-entrypoint-initdb.d/40-OpenWorld-rbac.sql (+ dashboard/dashboard-role.sql
-- for OpenWorld_dashboard). It matches docs/abac-rls-security-design.md §7 and the
...
-- The two AFTER-INSERT triggers on OpenWorld.messages call df.start (a durable
...
ALTER TABLE OpenWorld.messages DISABLE TRIGGER messages_outbound_send_trigger;
ALTER TABLE OpenWorld.messages DISABLE TRIGGER messages_user_loop_trigger;
...
--
-- GUCs (OpenWorld.current_agent_id, .current_chat_id, .current_user_id) are set
