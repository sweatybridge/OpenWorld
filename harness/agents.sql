...
-- host. Idempotent: ON CONFLICT DO NOTHING means re-runs are no-ops and the key
-- is never rotated. Skipped entirely when ATTOBOT_SSH_HOST/ATTOBOT_SSH_USER are
-- unset. The public key is RETURNING-ed only on first registration so the
-- operator can paste it into the remote ~/.ssh/authorized_keys (it lands in the
-- agent-init logs); recover it any time with
--   SELECT public_key FROM ssh.hosts WHERE host_name = :'ssh_host_name';
...
SELECT :'ssh_host_name', :'ssh_host', :'ssh_port'::integer, :'ssh_user',
 ...
 RETURNING host_name, public_key;
