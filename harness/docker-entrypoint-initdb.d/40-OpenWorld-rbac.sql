...
--     (e.g. OpenWorld_anonymous gets INSERT,UPDATE,SELECT on messages — never
...
--   * Adds OpenWorld.set_context(...) for session attribute bootstrap.
--
...
BEGIN
  -- One role per agent
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'OpenWorld_agent_primary') THEN
    CREATE ROLE OpenWorld_agent_primary NOLOGIN;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'OpenWorld_agent_sidecar') THEN
    CREATE ROLE OpenWorld_agent_sidecar NOLOGIN;
  END IF;

  -- Step down to user tiers when calling tools
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'OpenWorld_authenticated') THEN
    CREATE ROLE OpenWorld_authenticated NOLOGIN;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'OpenWorld_anonymous') THEN
    CREATE ROLE OpenWorld_anonymous NOLOGIN;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'OpenWorld_service') THEN
    CREATE ROLE OpenWorld_service NOLOGIN;
  END IF;
END $$;
...
ALTER ROLE OpenWorld_agent_primary LOGIN;
ALTER ROLE OpenWorld_agent_sidecar LOGIN;
ALTER ROLE OpenWorld_service BYPASSRLS;

-- ============================================================================
-- SCHEMA USAGE
-- ============================================================================

GRANT USAGE ON SCHEMA OpenWorld, OpenWorld_tools
  TO OpenWorld_authenticated, OpenWorld_anonymous, OpenWorld_service,
     OpenWorld_agent_primary, OpenWorld_agent_sidecar;
...
GRANT USAGE ON SEQUENCE OpenWorld.messages_id_seq, OpenWorld.memory_id_seq, OpenWorld.users_id_seq
  TO OpenWorld_agent_primary, OpenWorld_agent_sidecar;
-- Service role can add new models, agents, memory, or users through SQL tool 
GRANT USAGE ON SEQUENCE OpenWorld.agents_id_seq, OpenWorld.models_id_seq, OpenWorld.memory_id_seq, OpenWorld.users_id_seq
  TO OpenWorld_service;

-- ============================================================================
-- TABLE: OpenWorld.agents   (everyone reads; only service writes)
-- ============================================================================

GRANT SELECT ON OpenWorld.agents
  TO OpenWorld_anonymous, OpenWorld_authenticated, OpenWorld_service,
     OpenWorld_agent_primary, OpenWorld_agent_sidecar;
GRANT INSERT, UPDATE, DELETE ON OpenWorld.agents TO OpenWorld_service;

ALTER TABLE OpenWorld.agents ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS agents_read_all ON OpenWorld.agents;
CREATE POLICY agents_read_all ON OpenWorld.agents
  FOR SELECT TO PUBLIC USING (true);

-- ============================================================================
-- TABLE: OpenWorld.models   (everyone reads; only service writes)
-- ============================================================================

GRANT SELECT ON OpenWorld.models
  TO OpenWorld_anonymous, OpenWorld_authenticated, OpenWorld_service,
     OpenWorld_agent_primary, OpenWorld_agent_sidecar;
GRANT INSERT, UPDATE, DELETE ON OpenWorld.models TO OpenWorld_service;

ALTER TABLE OpenWorld.models ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS models_read_all ON OpenWorld.models;
CREATE POLICY models_read_all ON OpenWorld.models
  FOR SELECT TO PUBLIC USING (true);

-- ============================================================================
-- TABLE: OpenWorld.messages
...
GRANT SELECT ON OpenWorld.messages
  TO OpenWorld_anonymous, OpenWorld_authenticated;
GRANT SELECT, INSERT, UPDATE ON OpenWorld.messages
  TO OpenWorld_agent_primary, OpenWorld_agent_sidecar;
GRANT SELECT, INSERT, UPDATE, DELETE ON OpenWorld.messages
  TO OpenWorld_service;

ALTER TABLE OpenWorld.messages ENABLE ROW LEVEL SECURITY;

-- anonymous / authenticated: SELECT is chat-wide.
DROP POLICY IF EXISTS messages_user_select ON OpenWorld.messages;
CREATE POLICY messages_user_select ON OpenWorld.messages
  FOR SELECT TO OpenWorld_anonymous, OpenWorld_authenticated
  USING (
    agent_id = NULLIF(current_setting('OpenWorld.current_agent_id', true), '')::bigint
    AND chat_id = NULLIF(current_setting('OpenWorld.current_chat_id', true), '')::text
  );
...
DROP POLICY IF EXISTS messages_agent_all_own ON OpenWorld.messages;
CREATE POLICY messages_agent_all_own ON OpenWorld.messages
  FOR ALL TO OpenWorld_agent_primary, OpenWorld_agent_sidecar
  USING (agent_id = NULLIF(current_setting('OpenWorld.current_agent_id', true), '')::bigint)
  WITH CHECK (agent_id = NULLIF(current_setting('OpenWorld.current_agent_id', true), '')::bigint);

-- ============================================================================
-- TABLE: OpenWorld.memory
--   primary owns its memory; sidecar reviews and corrects memory across
--   ALL agents (read + insert + update); service full.
-- ============================================================================

GRANT SELECT, INSERT, UPDATE, DELETE ON OpenWorld.memory
  TO OpenWorld_agent_primary, OpenWorld_agent_sidecar, OpenWorld_service;

ALTER TABLE OpenWorld.memory ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS memory_primary_all_own ON OpenWorld.memory;
CREATE POLICY memory_primary_all_own ON OpenWorld.memory
  FOR ALL TO OpenWorld_agent_primary
  USING (agent_id = NULLIF(current_setting('OpenWorld.current_agent_id', true), '')::bigint)
  WITH CHECK (agent_id = NULLIF(current_setting('OpenWorld.current_agent_id', true), '')::bigint);

-- sidecar reads / writes every agent's memory (to review)...
DROP POLICY IF EXISTS memory_sidecar_bypass ON OpenWorld.memory;
CREATE POLICY memory_sidecar_bypass ON OpenWorld.memory
  FOR ALL TO OpenWorld_agent_sidecar USING (true) WITH CHECK (true);

-- ============================================================================
-- TABLE: OpenWorld.memory_sources
--   Junction backing memory's source messages (the composite FKs enforce
--   same-agent integrity and cascade). Ownership mirrors OpenWorld.memory: primary
--   owns its own, sidecar reads and corrects source links across ALL agents,
--   service full.
-- ============================================================================

GRANT SELECT, INSERT, UPDATE, DELETE ON OpenWorld.memory_sources
  TO OpenWorld_agent_primary, OpenWorld_agent_sidecar, OpenWorld_service;

ALTER TABLE OpenWorld.memory_sources ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS memory_sources_primary_all_own ON OpenWorld.memory_sources;
CREATE POLICY memory_sources_primary_all_own ON OpenWorld.memory_sources
  FOR ALL TO OpenWorld_agent_primary
  USING (agent_id = NULLIF(current_setting('OpenWorld.current_agent_id', true), '')::bigint)
  WITH CHECK (agent_id = NULLIF(current_setting('OpenWorld.current_agent_id', true), '')::bigint);

-- sidecar reads / writes every agent's source links...
DROP POLICY IF EXISTS memory_sources_sidecar_bypass ON OpenWorld.memory_sources;
CREATE POLICY memory_sources_sidecar_bypass ON OpenWorld.memory_sources
  FOR ALL TO OpenWorld_agent_sidecar USING (true) WITH CHECK (true);

-- ============================================================================
-- TABLE: OpenWorld.config
...
--   OpenWorld_service (sidecar). Neither can read secrets:
--     * the user tiers are granted SELECT on non-secret own rows only (policy
--       below); and
--     * OpenWorld_service — the sidecar's broad LLM-SQL tool scope — gets
...
GRANT SELECT ON OpenWorld.config
  TO OpenWorld_anonymous, OpenWorld_authenticated;
GRANT SELECT, INSERT, UPDATE ON OpenWorld.config
  TO OpenWorld_agent_primary, OpenWorld_agent_sidecar;
-- OpenWorld_service (the sidecar's LLM-SQL tool scope) must stay secret-free.
...
CREATE OR REPLACE VIEW OpenWorld.config_public AS
  SELECT agent_id, key, value, updated_at FROM OpenWorld.config WHERE NOT secret;
GRANT SELECT ON OpenWorld.config_public TO OpenWorld_service;

ALTER TABLE OpenWorld.config ENABLE ROW LEVEL SECURITY;
...
DROP POLICY IF EXISTS config_agent_write_own ON OpenWorld.config;
CREATE POLICY config_agent_write_own ON OpenWorld.config
  FOR ALL TO OpenWorld_agent_primary, OpenWorld_agent_sidecar
  USING (agent_id = NULLIF(current_setting('OpenWorld.current_agent_id', true), '')::bigint)
  WITH CHECK (agent_id = NULLIF(current_setting('OpenWorld.current_agent_id', true), '')::bigint);

-- user roles: read non-secret config rows only (e.g. model_id, not api_key)
DROP POLICY IF EXISTS config_user_read_nonsecret ON OpenWorld.config;
CREATE POLICY config_user_read_nonsecret ON OpenWorld.config
  FOR SELECT TO OpenWorld_anonymous, OpenWorld_authenticated
  USING (
    agent_id = NULLIF(current_setting('OpenWorld.current_agent_id', true), '')::bigint
    AND secret = false
...
-- ============================================================================
-- TABLE: OpenWorld.users   (channel identity ledger; intake upserts telegram users)
...
GRANT SELECT ON OpenWorld.users
  TO OpenWorld_anonymous, OpenWorld_authenticated, OpenWorld_agent_sidecar;
GRANT SELECT, INSERT, UPDATE ON OpenWorld.users
  TO OpenWorld_agent_primary;
GRANT SELECT, INSERT, UPDATE, DELETE ON OpenWorld.users
  TO OpenWorld_service;

ALTER TABLE OpenWorld.users ENABLE ROW LEVEL SECURITY;

-- user sees only their own row (resolved by the internal id in their session)
DROP POLICY IF EXISTS users_user_select_own ON OpenWorld.users;
CREATE POLICY users_user_select_own ON OpenWorld.users
  FOR SELECT TO OpenWorld_anonymous, OpenWorld_authenticated
  USING (id = NULLIF(current_setting('OpenWorld.current_user_id', true), '')::bigint);

-- agent reads all users (resolve the requesting user during a turn)
DROP POLICY IF EXISTS users_agent_read_all ON OpenWorld.users;
CREATE POLICY users_agent_read_all ON OpenWorld.users
  FOR SELECT TO OpenWorld_agent_primary, OpenWorld_agent_sidecar
  USING (true);

-- OpenWorld_agent_primary creates and edits users (no DELETE). The ledger has no
-- agent_id, so management is global; it can promote tier, but not delete rows.
DROP POLICY IF EXISTS users_primary_insert ON OpenWorld.users;
CREATE POLICY users_primary_insert ON OpenWorld.users
  FOR INSERT TO OpenWorld_agent_primary WITH CHECK (true);

DROP POLICY IF EXISTS users_primary_update ON OpenWorld.users;
CREATE POLICY users_primary_update ON OpenWorld.users
  FOR UPDATE TO OpenWorld_agent_primary USING (true) WITH CHECK (true);

-- ============================================================================
-- FUNCTION EXECUTE PRIVILEGES
--   Per-function for the verified OpenWorld entrypoints (least privilege, and
--   to document the intent). Schema-wide for OpenWorld_tools (the agent tooling
--   surface) and for the trusted service role.
-- ============================================================================

-- Agent roles now run their own loops, so they also need the full OpenWorld
-- function surface (compose_llm_request, record_assistant, poll_messages,
-- upsert_user, helpers, ...).
GRANT EXECUTE ON ALL FUNCTIONS IN SCHEMA OpenWorld
  TO OpenWorld_agent_primary, OpenWorld_agent_sidecar;
...
GRANT EXECUTE ON ALL FUNCTIONS IN SCHEMA OpenWorld_tools
  TO OpenWorld_anonymous, OpenWorld_authenticated, OpenWorld_service,
     OpenWorld_agent_primary, OpenWorld_agent_sidecar;
...
-- TODO: move this function to OpenWorld_tools or telegram schema
GRANT EXECUTE ON FUNCTION OpenWorld.queue_outbound_attachment(text, text, text, text, text, text, text)
  TO OpenWorld_anonymous, OpenWorld_authenticated;
...
--   get include_http. OpenWorld_service no longer submits workflows, so it gets no
--   df access.
--   (OpenWorld_anonymous / OpenWorld_authenticated are intentionally NOT granted df
--    access: end users interact through the agent, not by submitting durable
--    workflows themselves.)
-- ============================================================================
SELECT df.grant_usage('OpenWorld_service',            include_http => true);
SELECT df.grant_usage('OpenWorld_agent_primary',      include_http => true);
SELECT df.grant_usage('OpenWorld_agent_sidecar', include_http => true);
...
--   sidecar -> OpenWorld_service (broad, secret-free)
GRANT OpenWorld_anonymous TO OpenWorld_agent_primary;
GRANT OpenWorld_authenticated TO OpenWorld_agent_primary;
GRANT OpenWorld_service TO OpenWorld_agent_sidecar;
...
-- caller is the loop / message-inserter role OpenWorld_agent_primary, so df.start
...
ALTER FUNCTION OpenWorld.ensure_telegram_inbox_loop(text, integer)       OWNER TO OpenWorld_agent_primary;
ALTER FUNCTION OpenWorld.ensure_agent_cron_loop(text, text, text, text)  OWNER TO OpenWorld_agent_sidecar;
ALTER FUNCTION OpenWorld.queue_outbound_attachment(text, text, text, text, text, text, text) OWNER TO OpenWorld_agent_primary;
...
CREATE OR REPLACE FUNCTION OpenWorld.set_context(
  p_role text,
...
AS $$
BEGIN
  PERFORM set_config('OpenWorld.current_role', p_role, true);
  PERFORM set_config('OpenWorld.current_agent_id', COALESCE(p_agent_id::text, ''), true);
  PERFORM set_config('OpenWorld.current_chat_id', COALESCE(p_telegram_chat_id, ''), true);
  PERFORM set_config('OpenWorld.current_user_id', COALESCE(p_user_id::text, ''), true);
  PERFORM set_config('OpenWorld.current_channel', COALESCE(p_channel, ''), true);
END;
$$;
