...
CREATE SCHEMA IF NOT EXISTS ow_tools;

CREATE TABLE IF NOT EXISTS ow.models (
...
CREATE TABLE IF NOT EXISTS ow.agents (
  id bigserial PRIMARY KEY,
  slug text NOT NULL UNIQUE,
  soul text NOT NULL DEFAULT '',
  model_id bigint NOT NULL REFERENCES ow.models(id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS ow.config (
  agent_id bigint NOT NULL REFERENCES ow.agents(id) ON DELETE CASCADE,
  key text NOT NULL,
...
CREATE TABLE IF NOT EXISTS ow.messages (
  id bigserial PRIMARY KEY,
  agent_id bigint NOT NULL REFERENCES ow.agents(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS memory_agent_enabled_id_idx
  ON ow.memory(agent_id, enabled, id);
...
ALTER TABLE ow.messages ADD CONSTRAINT messages_agent_id_id_key UNIQUE (agent_id, id);
ALTER TABLE ow.memory   ADD CONSTRAINT memory_id_agent_key     UNIQUE (id, agent_id);
...
CREATE TABLE IF NOT EXISTS ow.memory_sources (
  memory_id  bigint NOT NULL,
  agent_id   bigint NOT NULL,
  message_id bigint NOT NULL,
  PRIMARY KEY (memory_id, message_id),
  FOREIGN KEY (memory_id, agent_id)
    REFERENCES ow.memory(id, agent_id) ON DELETE CASCADE,
  FOREIGN KEY (agent_id, message_id)
    REFERENCES ow.messages(agent_id, id) ON DELETE CASCADE
);
...
-- `tier` maps the user to an RLS role suffix (ow_<tier>); it defaults to
-- 'anonymous' and is promoted to 'authenticated' by an operator.
CREATE TABLE IF NOT EXISTS ow.users (
  id bigserial PRIMARY KEY,
