...
--
-- start_agent_loop (30-attobot-durable.sql) now embeds the trigger message id
-- into the loop label (attobot:<slug>:loop:<msg_id>), so every instance in a
-- turn shares a message-id key. This file adds:
--   * attobot.parse_instance_label — pure label → (kind, agent, msg, tool_call)
...
--   * attobot.instance_index       — structured projection of df.instances via
--     the parser. security_invoker, so it sees what the caller sees.
--   * attobot.turn_trigger_id      — given any message id in a turn, resolve the
--     turn's trigger (the owning user/system message). Pure, unit tested.
--   * attobot.trace_turn(msg)      — the full instance tree for a turn (loop +
...
-- attobot_dashboard role already has BYPASSRLS + USAGE on df + EXECUTE on the
...
-- backward-compatible: the loop arm matches both legacy `attobot:x:loop` and
-- the new `attobot:x:loop:<id>`.
CREATE OR REPLACE FUNCTION attobot.parse_instance_label(p_label text)
RETURNS TABLE(
  kind         text,
  agent_slug   text,
  message_id   bigint,
...
AS $$
  SELECT
    CASE
      WHEN p_label ~ '^attobot:[^:]+:loop($|:)' THEN 'loop'
      WHEN p_label ~ '^attobot:[^:]+:inbox$'    THEN 'inbox'
      WHEN p_label ~ '^attobot:[^:]+:cron:'     THEN 'cron'
      WHEN p_label ~ '^attobot:send:'           THEN 'send'
      WHEN p_label ~ '^attobot:tool:'           THEN 'tool'
      WHEN p_label ~ '^attobot:typing:'         THEN 'typing'
      WHEN p_label LIKE 'attobot:%'             THEN 'attobot'
      ELSE 'other'
    END,
    COALESCE(
      (regexp_match(p_label, '^attobot:([^:]+):loop($|:)'))[1],
      (regexp_match(p_label, '^attobot:([^:]+):inbox$'))[1],
      (regexp_match(p_label, '^attobot:([^:]+):cron:'))[1]
    ),
    CASE
      WHEN p_label ~ '^attobot:[^:]+:loop:' THEN NULLIF((regexp_match(p_label, '^attobot:[^:]+:loop:([0-9]+)'))[1], '')::bigint
      WHEN p_label ~ '^attobot:typing:'     THEN NULLIF((regexp_match(p_label, '^attobot:typing:([0-9]+)'))[1], '')::bigint
      WHEN p_label ~ '^attobot:send:'       THEN NULLIF((regexp_match(p_label, '^attobot:send:([0-9]+)'))[1], '')::bigint
      WHEN p_label ~ '^attobot:tool:'       THEN NULLIF((regexp_match(p_label, '^attobot:tool:([0-9]+):'))[1], '')::bigint
      ELSE NULL
    END,
    CASE
      WHEN p_label ~ '^attobot:tool:' THEN (regexp_match(p_label, '^attobot:tool:[0-9]+:(.+)$'))[1]
      ELSE NULL
    END;
$$;
...
CREATE OR REPLACE VIEW attobot.instance_index
...
FROM df.instances i,
     LATERAL attobot.parse_instance_label(i.label) AS p;
...
CREATE OR REPLACE FUNCTION attobot.turn_trigger_id(p_message_id bigint)
RETURNS bigint
LANGUAGE plpgsql
STABLE
SECURITY INVOKER
SET search_path = attobot, public, pg_temp
AS $$
DECLARE
  v_role      text;
  v_agent_id  bigint;
...
CREATE OR REPLACE FUNCTION attobot._instance_result(p_id text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY INVOKER
SET search_path = attobot, public, pg_temp
...
CREATE OR REPLACE FUNCTION attobot.trace_turn(p_message_id bigint)
RETURNS TABLE(
  instance_id  text,
  kind         text,
  agent_slug   text,
...
BEGIN
  v_trigger_id := attobot.turn_trigger_id(p_message_id);
  IF v_trigger_id IS NULL THEN
    RETURN;  -- unknown message → empty trace
  END IF;

  SELECT agent_id INTO v_agent_id FROM attobot.messages WHERE id = v_trigger_id;
  ...
  SELECT ix.id::text, ix.kind, ix.agent_slug, ix.message_id, ix.tool_call_id,
         ix.status::text, ix.updated_at, attobot._instance_result(ix.id) AS result
  FROM attobot.instance_index ix
  WHERE ix.message_id = v_trigger_id                       -- loop + typing for the trigger
     OR ix.message_id IN (                                  -- tool/send for each assistant msg
          SELECT m.id FROM attobot.messages m
