...
-- into the loop label (OpenWorld:<slug>:loop:<msg_id>), so every instance in a
-- turn shares a message-id key. This file adds:
--   * OpenWorld.parse_instance_label — pure label → (kind, agent, msg, tool_call)
...
--   * OpenWorld.instance_index       — structured projection of df.instances via
--     the parser. security_invoker, so it sees what the caller sees.
--   * OpenWorld.turn_trigger_id      — given any message id in a turn, resolve the
--     turn's trigger (the owning user/system message). Pure, unit tested.
--   * OpenWorld.trace_turn(msg)      — the full instance tree for a turn (loop +
...
-- OpenWorld_dashboard role already has BYPASSRLS + USAGE on df + EXECUTE on the
...
-- backward-compatible: the loop arm matches both legacy `OpenWorld:x:loop` and
-- the new `OpenWorld:x:loop:<id>`.
CREATE OR REPLACE FUNCTION OpenWorld.parse_instance_label(p_label text)
RETURNS TABLE(
  kind         text,
  agent_slug   text,
  message_id   bigint,
...
AS $$
  SELECT
    CASE
      WHEN p_label ~ '^OpenWorld:[^:]+:loop($|:)' THEN 'loop'
      WHEN p_label ~ '^OpenWorld:[^:]+:inbox$'    THEN 'inbox'
      WHEN p_label ~ '^OpenWorld:[^:]+:cron:'     THEN 'cron'
      WHEN p_label ~ '^OpenWorld:send:'           THEN 'send'
      WHEN p_label ~ '^OpenWorld:tool:'           THEN 'tool'
      WHEN p_label ~ '^OpenWorld:typing:'         THEN 'typing'
      WHEN p_label LIKE 'OpenWorld:%'             THEN 'OpenWorld'
      ELSE 'other'
    END,
    COALESCE(
      (regexp_match(p_label, '^OpenWorld:([^:]+):loop($|:)'))[1],
      (regexp_match(p_label, '^OpenWorld:([^:]+):inbox$'))[1],
      (regexp_match(p_label, '^OpenWorld:([^:]+):cron:'))[1]
    ),
    CASE
      WHEN p_label ~ '^OpenWorld:[^:]+:loop:' THEN NULLIF((regexp_match(p_label, '^OpenWorld:[^:]+:loop:([0-9]+)'))[1], '')::bigint
      WHEN p_label ~ '^OpenWorld:typing:'     THEN NULLIF((regexp_match(p_label, '^OpenWorld:typing:([0-9]+)'))[1], '')::bigint
      WHEN p_label ~ '^OpenWorld:send:'       THEN NULLIF((regexp_match(p_label, '^OpenWorld:send:([0-9]+)'))[1], '')::bigint
      WHEN p_label ~ '^OpenWorld:tool:'       THEN NULLIF((regexp_match(p_label, '^OpenWorld:tool:([0-9]+):'))[1], '')::bigint
      ELSE NULL
    END,
    CASE
      WHEN p_label ~ '^OpenWorld:tool:' THEN (regexp_match(p_label, '^OpenWorld:tool:[0-9]+:(.+)$'))[1]
      ELSE NULL
    END;
$$;
...
CREATE OR REPLACE VIEW OpenWorld.instance_index
...
FROM df.instances i,
     LATERAL OpenWorld.parse_instance_label(i.label) AS p;
...
CREATE OR REPLACE FUNCTION OpenWorld.turn_trigger_id(p_message_id bigint)
RETURNS bigint
LANGUAGE plpgsql
STABLE
SECURITY INVOKER
SET search_path = OpenWorld, public, pg_temp
AS $$
DECLARE
  v_role      text;
  v_agent_id  bigint;
...
BEGIN
  SELECT role, agent_id INTO v_role, v_agent_id
    FROM OpenWorld.messages WHERE id = p_message_id;
  IF NOT FOUND THEN
    RETURN NULL;
  END IF;
...
CREATE OR REPLACE FUNCTION OpenWorld._instance_result(p_id text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY INVOKER
SET search_path = OpenWorld, public, pg_temp
...
CREATE OR REPLACE FUNCTION OpenWorld.trace_turn(p_message_id bigint)
RETURNS TABLE(
  instance_id  text,
  kind         text,
  agent_slug   text,
...
BEGIN
  v_trigger_id := OpenWorld.turn_trigger_id(p_message_id);
  IF v_trigger_id IS NULL THEN
    RETURN;  -- unknown message → empty trace
  END IF;

  SELECT agent_id INTO v_agent_id FROM OpenWorld.messages WHERE id = v_trigger_id;
  ...
  SELECT ix.id::text, ix.kind, ix.agent_slug, ix.message_id, ix.tool_call_id,
         ix.status::text, ix.updated_at, OpenWorld._instance_result(ix.id) AS result
  FROM OpenWorld.instance_index ix
  WHERE ix.message_id = v_trigger_id                       -- loop + typing for the trigger
     OR ix.message_id IN (                                  -- tool/send for each assistant msg
          SELECT m.id FROM OpenWorld.messages m
